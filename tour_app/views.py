...
def home(request):
    ...
    if 'package_obj' in request.session:
        ...
        package_obj=packages.objects.filter(trending=True)
        print(package_obj)
...
class search(ListView):
    ...
    def get_queryset(self):
        ...
        if query:
            ...
            employees = packages.objects.all()
            employee_names = [employee.p1_name for employee in employees]

            # Find the best match using fuzzy matching
            best_match = process.extractOne(query, employee_names, scorer=fuzz.ratio)

            # If a good match is found (above a threshold, e.g. 70% similarity)
            if best_match and best_match[1] > 65:  # threshold 70%
                suggestion = best_match[0]  # closest match
            else:
                suggestion="no suggestions matches"

            # Return filtered results if necessary (optional)
            objects = packages.objects.filter(p1_name=query)

            object_list = list(objects)
            # Append the string as a separate value to the list of objects
            if objects.exists():
                object_list.append({"suggestion":None})
            else:
                object_list.append({"suggestion":suggestion})
        ...
        if request.method == "POST":
            ...
            try:
                ...
                if con == "on":
                    ...
                    booking_to_confirm.confirm = False
                    booking_to_confirm.save()
            except user_booking.DoesNotExist:
                messages.error(request, "Booking not found or you are not authorized to confirm it.")

        user_bookings = user_booking.objects.filter(Q(user=request.user) )
        
        total_adult_price = sum(i.Adult_price for i in user_bookings if i.confirm==True)
        total_child_price = sum(i.child_price for i in user_bookings if i.confirm==True)
